...
public class SinglePassAnalyzerWithSimpleVerifierTest extends AsmTest {
    ...
    public void testAnalyze_simpleVerifier(final AsmTest.PrecompiledClass classParameter, final AsmTest.Api apiParameter) throws AnalyzerException {
        ClassNode classNode = computeFrames(classParameter);
        assumeFalse(classNode.methods.isEmpty());
        Analyzer<BasicValue> baseAnalyzer = new Analyzer<BasicValue>(
                new SimpleVerifier(
                    Type.getObjectType(classNode.name),
                    Type.getObjectType(classNode.superName),
                    (classNode.access & Opcodes.ACC_INTERFACE) != 0));

        SinglePassAnalyzer frameAnalyzer = new SinglePassAnalyzer(
            new SimpleVerifier(
                Type.getObjectType(classNode.name),
                Type.getObjectType(classNode.superName),
                (classNode.access & Opcodes.ACC_INTERFACE) != 0));
        ...
        for (MethodNode methodNode : classNode.methods) {
            ...
            for (int i = 0; i < framesA.length; i++) {
                try {
                    ...
                    throw new AssertionError("Error in " + classNode.name + "." + methodNode.name + methodNode.desc + " at instruction " + i, e);
                }
            }
        }
    }

    private void compareFrames(Frame<BasicValue> frameA, Frame<BasicValue> frameB, int idx) {
        ...
        if (localsA != localsB) {
            throw new AssertionError("Different number of locals at " + idx + " \n" + frameA + "\n" + frameB);
        }
        if (stackA != stackB) {
            throw new AssertionError("Different number of stack elements at " + idx + " \n" + frameA + "\n" + frameB);
        }
        for (int i = 0; i < localsA; i++) {
            if (!frameA.getLocal(i).equals(frameB.getLocal(i))) {
                throw new AssertionError("Different local at " + idx + "  at index " + i + "\n" + frameA + "\n" + frameB);
            }
        }
        for (int i = 0; i < stackA; i++) {
            if (!frameA.getStack(i).equals(frameB.getStack(i))) {
                throw new AssertionError("Different stack element at " + idx + "  at index " + i + "\n" + frameA + "\n" + frameB);
            }
        }
    }
